...
num_classes = 1
shufflNet_wcw_model = ShuffleNetV2(input_shape=input_shape, num_classes=num_classes, model_size='1.0x')
shufflNet_wcw_model.compile(optimizer=opt, loss='binary_crossentropy', metrics=['accuracy'])
    
wcw_model_checkpoint = keras.callbacks.ModelCheckpoint(filepath='/WACV_Paper/Models_RAW/ShuffleNet_Diff_wCW.keras', save_best_only=True, monitor='val_accuracy', mode='max', verbose=1)
...
wcw_history_df.to_csv('/WACV_Paper/History/ShuffleNet_Diff_wCW.csv', index=False)
...
shufflNet_cw_model.compile(optimizer=opt, loss='binary_crossentropy', metrics=['accuracy'])
...
file_path = '/WACV_Paper/Plots_RAW/ShuffleNet_Diff_wCW_PR_Curve.csv'
...
plt.legend()
plt.grid(True)
precision_recall_curve_path = '/WACV_Paper/Plots_RAW/ShuffleNet_Diff_wCW_PR_Curve.png'
...
test_loss, test_acc = shufflNet_cw_model.evaluate(X_test, y_test)
test_acc  = test_acc *100

predictions = shufflNet_cw_model.predict(X_test)
predicted_labels = np.argmax(predictions, axis=1)

precision, recall, _ = precision_recall_curve(y_test, predictions[:, 0])

pr_data = pd.DataFrame({'Precision': precision, 'Recall': recall })
file_path = '/WACV_Paper/Plots_RAW/ShuffleNet_Diff_CW_PR_Curve.csv'
pr_data.to_csv(file_path, index=False)
...
plt.legend()
plt.grid(True)
precision_recall_curve_path = '/WACV_Paper/Plots_RAW/ShuffleNet_Diff_CW_PR_Curve.png'
