...
from tensorflow.keras.layers import Input, Conv2D, Add, DepthwiseConv2D, ReLU, GlobalAveragePooling2D
from keras.layers import MaxPooling2D, Flatten, Dense, Dropout, BatchNormalization, concatenate
...
def create_resnet50_model(input_shape=(224,224, 1)):
    ...
    x = resnet_conv_block(x, filters=[64, 64, 256], kernel_size=(3, 3), strides=(1, 1))
    for _ in range(2):
        x = resnet_identity_block(x, filters=[64, 64, 256], kernel_size=(3, 3))

    x = resnet_conv_block(x, filters=[128, 128, 512], kernel_size=(3, 3), strides=(2, 2))
    for _ in range(3):
        x = resnet_identity_block(x, filters=[128, 128, 512], kernel_size=(3, 3))

    x = resnet_conv_block(x, filters=[256, 256, 1024], kernel_size=(3, 3), strides=(2, 2))
    for _ in range(5):
        x = resnet_identity_block(x, filters=[256, 256, 1024], kernel_size=(3, 3))

    x = resnet_conv_block(x, filters=[512, 512, 2048], kernel_size=(3, 3), strides=(2, 2))
    for _ in range(2):
        x = resnet_identity_block(x, filters=[512, 512, 2048], kernel_size=(3, 3))
...
resnet50_wcw_model = create_resnet50_model()
resnet50_wcw_model.compile(optimizer=opt, loss='binary_crossentropy', metrics=['accuracy'])
    
wcw_model_checkpoint = keras.callbacks.ModelCheckpoint(filepath='/WACV_Paper/Models_RAW/HIGH_ResNet50_RAW_wCW.keras', save_best_only=True, monitor='val_accuracy', mode='max', verbose=1)
...
wcw_history_df.to_csv('/WACV_Paper/History_RAW/HIGH_ResNet50_RAW_wCW.csv', index=False)
...
# resnet50_cw_model.compile(optimizer=opt, loss='binary_crossentropy', metrics=['accuracy'])

# cw_model_checkpoint = keras.callbacks.ModelCheckpoint(filepath='/WACV_Paper/Models_RAW/ResNet50_Diff_CW.keras', save_best_only=True, monitor='val_accuracy', mode='max', verbose=1)
...
# cw_history_df.to_csv('/WACV_Paper/History/ResNet50_Diff_CW.csv', index=False)
##########################################################################################################################################################################
##########################################################################################################################################################################
                                                                    # Testing
##########################################################################################################################################################################
...
test_loss, test_acc = resnet50_wcw_model.evaluate(X_test, y_test)
test_acc  = test_acc *100

predictions = resnet50_wcw_model.predict(X_test)
...
file_path = '/WACV_Paper/Plots_CSV/HIGH_ResNet50_RAW_wCW_PR_Curve.csv'
...
plt.legend()
plt.grid(True)
precision_recall_curve_path = '/WACV_Paper/Plots_RAW/HIGH_ResNet50_RAW_wCW_PR_Curve.png'
...
# test_loss, test_acc = resnet50_cw_model.evaluate(X_test, y_test)
# test_acc  = test_acc *100

# predictions = resnet50_cw_model.predict(X_test)
# predicted_labels = np.argmax(predictions, axis=1)

# precision, recall, _ = precision_recall_curve(y_test, predictions[:, 0])

# pr_data = pd.DataFrame({'Precision': precision, 'Recall': recall })
# file_path = '/WACV_Paper/Plots_RAW/ResNet50_Diff_CW_PR_Curve.csv'
# pr_data.to_csv(file_path, index=False)
...
# plt.legend()
# plt.grid(True)
# precision_recall_curve_path = '/WACV_Paper/Plots_RAW/ResNet50_Diff_CW_PR_Curve.png'
